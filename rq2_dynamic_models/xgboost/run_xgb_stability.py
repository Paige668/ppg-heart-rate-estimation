"""
Run 5-seed stability analysis for the strict-matched XGBoost baseline.

This keeps the strict matched feature pipeline and validation split fixed,
and varies only the XGBoost seed.
"""
...
from run_xgb_matched import run_xgb_experiment
...
def run_case(case_slug, use_full_pipeline):
    ...
    for training_seed in TRAINING_SEEDS:
        ...
        result = run_xgb_experiment(
            ...
            output_dir=seed_dir,
            prediction_file_name=f"xgb_strict_{case_slug}_seed_{training_seed}.csv",
        )
        rows.append(result)
...
def summarize(df):
    return {
        ...
        "best_round_mean": df["best_round"].mean(),
        "best_round_std": df["best_round"].std(ddof=0),
    }
...
def write_summary(no_prep_df, full_pipeline_df):
    ...
    markdown = f"""# XGBoost Strict Matched Stability Analysis

Validation split seed: {SPLIT_SEED}

Model seeds used: {', '.join(str(seed) for seed in TRAINING_SEEDS)}

| Case | Test MAE mean ± std | Test RMSE mean ± std | Best round mean ± std |
| ---- | ---- | ---- | ---- |
| No-Prep | {no_stats['mae_mean']:.4f} ± {no_stats['mae_std']:.4f} | {no_stats['rmse_mean']:.4f} ± {no_stats['rmse_std']:.4f} | {no_stats['best_round_mean']:.1f} ± {no_stats['best_round_std']:.1f} |
| Full Pipeline | {fp_stats['mae_mean']:.4f} ± {fp_stats['mae_std']:.4f} | {fp_stats['rmse_mean']:.4f} ± {fp_stats['rmse_std']:.4f} | {fp_stats['best_round_mean']:.1f} ± {fp_stats['best_round_std']:.1f} |
"""
...
def main():
    ...
    plot_case(no_prep_df, "no_prep", "XGBoost Strict Matched Stability - No Prep")
    plot_case(full_pipeline_df, "full_pipeline", "XGBoost Strict Matched Stability - Full Pipeline")
